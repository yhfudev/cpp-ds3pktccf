...
#endif
        assert (NULL != this->pktlist[i].pkt);
...
ds3_packet_buffer_t *
ds3_packet_buffer_gpkt_t::insert_to_base (size_t pos_peer, ds3_packet_buffer_gpkt_t *peer, size_t begin_self, size_t end_self)
{
    assert (NULL != peer);
    // TODO: insert content from self to self
    ...
    for (; szcurj < end_self;) {
        ...
        memmove (&pi, &(this->pktlist[j]), sizeof(pi));
...
ds3_packet_buffer_t *
ds3_packet_buffer_gpkt_t::copy_to_base (size_t pos_peer, ds3_packet_buffer_gpkt_t * peer, size_t begin_self, size_t end_self)
{
    assert (NULL != peer);
    // TODO: copy content from self to self
	// copy the content between [begin_self, end_self) to peer
    if ((ssize_t)begin_self >= this->size()) {
        return NULL;
    }
    if ((ssize_t)end_self > this->size()) {
...
int
test_pktgnc_gp (ds3_test_exec_info_t *pinfo, size_t numpi)
{
...
static int
test_pktgnc_fix1 (void)
{
    ds3_test_exec_info_t testcase1[] = {
        { EXECODE_INSERT_FROM, 0 /* sq */,  1 /* maxsz */, 0 /* range_begin */, 5 /* range_end */, 0 /* pos_self */, },
        { EXECODE_INSERT_FROM, 1 /* sq */,  2 /* maxsz */, 0 /* range_begin */, 6 /* range_end */, 5 /* pos_self */, },
        { EXECODE_INSERT_FROM, 2 /* sq */,  3 /* maxsz */, 0 /* range_begin */, 7 /* range_end */, 11 /* pos_self */, },
        { EXECODE_INSERT_FROM, 3 /* sq */,  4 /* maxsz */, 0 /* range_begin */, 8 /* range_end */, 18 /* pos_self */, },
    };

    return test_pktgnc_gp (testcase1, NUMARRAY(testcase1));
}

static int
test_pktgnc_fix2 (void)
{
    ds3_test_exec_info_t testcase1[] = {
...
int
test_pktgnc_random (void)
{
    size_t nump = rand () % 10000;
    return test_pktgnc_gp (NULL, nump);
}

int
test_pktgnc (void)
{
    REQUIRE (0 == test_pktgnc_fix1());
    REQUIRE (0 == test_pktgnc_fix2());
    REQUIRE (0 == test_pktgnc_fix3());
    REQUIRE (0 == test_pktgnc_fix4());
    REQUIRE (0 == test_pktgnc_fix5());
    for (int i = 0; i < 10; i ++) {
        REQUIRE (0 == test_pktgnc_random());
    }
    return 0;
}
